const nepali = {
  //landing page
  landingMsg1: "रिपुराज सोनाशक्ति किन्नुहोस्",
  landingMsg2: "र",
  landingMsg3: "सुन",
  landingMsg4: "र",
  landingMsg5: "चाँदीका",
  landingMsg6: "सिक्का",
  landingMsg7: "निःशुल्क जित्नुहोस्",

  //Enter Phone and Coupon Steps
  enterMobileNumber: "आफ्नो मोबाइल नम्बर प्रविष्ट गर्नुहोस्",
  phoneNumber: "फोन नम्बर",
  termsText: "म सहमत छु",
  terms: "नियमहरू र गोपनीयता नीति",
  sendOtp: "ओटीपी पठाउनुहोस्",

  howToUseCoupon: "कुपन कसरी प्रयोग गर्ने:",
  step1:
    "आफ्नो मोबाइलबाट <0>QR कोड</0> <1>स्क्यान</1> गर्नुहोस् र <2>लिंक</2> क्लिक गर्नुहोस्।",
  step2:
    "खुलेको पेजमा आफ्नो नाम, पूरा ठेगाना, मोबाइल नम्बर, र स्क्र्याच कार्डमा दिइएको <0>कुपन कोड</0> प्रविष्ट गर्नुहोस्।",
  step3: "<0>सबमिट</0> बटनमा क्लिक गरी आफ्नो गुनासो वा सुझाव पठाउनुहोस्।",
  step4:
    "<0>SMS</0> र <1>WhatsApp</1> मा <2>Welcome Note</2> प्राप्त गर्नुहोस्।",

  //translation for video section
  usetheCoupon: "कुपन कसरी प्रयोग गर्ने ?",
  usetheCouponDetails:
    "अब पुरस्कार जित्न अझ सजिलो भयो! यो भिडियो हेर्नुहोस् र जान्नुहोस् कसरी तपाईं Ripuraj <1>Gold/Silver Scheme</1> मा भाग लिएर <2>भाग्यशाली विजेता</2> बन्न सक्नुहुन्छ!",
};

export default nepali;
